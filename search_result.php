...
if ($id!=""){
	$id_id=array();
	$result=mysqli_query($conn,"select id from book where id='$id'");
...
if ($year!=""){
	$year_id=array();
	$result=mysqli_query($conn,"select id from book where publish_year='$year'");
	while($row=mysqli_fetch_row($result)){
...
// 获得查询到的书的详细信息
for ($i=0;$i<$num;$i++){
	$bresult=mysqli_query($conn,"select * from book where id='$resultid[$i]'");
