	...
	if (isset($_POST['return'])){
		...
		for($i=0;$i<count($bookbox);$i++){
			...
			if ($bookbox[$i]==""){
				...
				mysqli_query($conn,$returnsql) or die ("删除借书记录失败：".mysqli_error());
				// 在lend_log表中记录还书时间
				// 获得当前日期
				$now = date("Y-m-d");
				$logsql="update lend_log set return_time='$now' where book_id='$book_id' and user_id='$user_id'";
				mysqli_query($conn,$logsql) or die ("记录还书时间失败：".mysqli_error());
				// 在book表中增加一本现存书数量
				$booksql="update book set leave_number=leave_number+1 where id='$book_id'";
				mysqli_query($conn,$booksql) or die ("增加剩余书数量失败：".mysqli_error());
				?>
				<p align="center">&nbsp;</p>
				<p align="center">&nbsp;</p>
	...
	if (isset($_POST['renew'])){
		...
		for($i=0;$i<count($bookbox);$i++){
			...
			if ($bookbox[$i]==""){
				...
				mysqli_query($conn,$renewsql) or die ("续借失败：".mysqli_error());
				?>
				<p align="center">&nbsp;</p>
				<p align="center">&nbsp;</p>
