...
<?php
// 提交前
if (!isset($_POST['show']) and !isset($_POST['lend'])){
?>
...
<?
	...
	if (isset($_POST['lend'])){
		...
		mysqli_query($conn,$lendsql) or die ("操作失败：".mysqli_error());
		// 还需要在log中记录
		$logsql="insert into lend_log(book_id,user_id,lend_time) values('$book_id','$user_id','$now')";
		mysqli_query($conn,$logsql) or die ("记录失败：".mysqli_error());
		// 借出后需要在该书记录中库存剩余数减一
        $leave = $_POST['leave'];
		$leave_num=$leave-1;
		mysqli_query($conn,"update book set leave_number='$leave_num' where id='$book_id'");
		?>
		<p align="center">&nbsp;</p>
		<p align="center">&nbsp;</p>
		<p align="center"><font color="red">借阅登记完成！</p>
		<p align="center"><a href="<?php echo $_SERVER['PHP_SELF']; ?>">继续添加</a></p>
<?
	}
}
